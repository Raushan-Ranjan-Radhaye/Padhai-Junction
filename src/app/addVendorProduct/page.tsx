...
function AddVendorProduct() {
  ...
  const [detailPoints, setDetailPoint] = useState<string[]>([]);
  ...
  const handleAddPoint = () => {
    ...
    setDetailPoint((prev) => [...prev, currentPoint]);
    setCurrentPoint("");
  };

  const handlerRemove = (i: number) => {
    setDetailPoint((prev) => prev.filter((_, index) => index !== i));
  };
      ...
      try {
        ...
        if (typeof error === 'object' && error !== null && 'response' in error) {
          ...
          if (axiosError.response?.status === 403) {
            alert(axiosError.response.data.message || "You must be a verified vendor to add products.");
