...
function VendorOrders() {
  ...
  const updateStatus = async (orderId: string, status: string) => {
    ...
    try {
      ...
      // Using a simple alert for now, but you could replace with a toast
  ...
  const handleStatusChange = async (order: any, newStatus: string) => {
    if (newStatus === "delivered") {
      // Call API to generate and send OTP first
      setIsLoading(true);
      try {
        ...
        if (result.data.message === "OTP sent successfully") {
          // Open OTP modal for delivery verification (status will be updated after OTP verification)
          setOtpModel(order);
          alert(
            "OTP has been sent to the buyer's email. Please ask the customer for the OTP.",
          );
        } else {
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 lg:gap-4 mb-4 sm:mb-6">
        <div className="bg-gradient-to-br from-white/10 to-white/5 border border-white/10 rounded-xl p-2 sm:p-3 lg:p-4">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              <p className="text-lg sm:text-xl lg:text-2xl font-bold truncate">{orders.length}</p>
        ...
        {/* <div className="bg-gradient-to-br from-white/10 to-white/5 border border-white/10 rounded-xl p-2 sm:p-3 lg:p-4">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              <p className="text-lg sm:text-xl lg:text-2xl font-bold truncate">
                {orders.filter((o: any) => o.orderStatus === "shipped").length}
              </p>
        ...
        {/* <div className="bg-gradient-to-br from-white/10 to-white/5 border border-white/10 rounded-xl p-2 sm:p-3 lg:p-4">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              <p className="text-lg sm:text-xl lg:text-2xl font-bold truncate">
                {orders.filter((o: any) => o.orderStatus === "delivered").length}
              </p>
          ...
          <table className="w-full min-w-[800px]">
            <thead>
              <tr className="bg-gradient-to-r from-white/10 to-white/5 border-b border-white/20">
                ...
                <th className="p-3 lg:p-4 text-center">
                  <span className="flex items-center justify-center gap-1.5 lg:gap-2 text-xs lg:text-sm font-semibold text-gray-300 uppercase tracking-wider">
                    <FaEdit className="text-xs text-pink-400" /> Update
                  </span>
                </th>
              </tr>
            </thead>
                    ...
                    <td className="p-2 lg:p-4">
                      {canUpdateStatus(order.orderStatus) ? (
                        <div className="flex items-center justify-center">
                          <select
                            className="bg-white/10 border border-white/20 rounded-lg px-2 lg:px-3 py-1.5 lg:py-2 text-white text-xs lg:text-sm cursor-pointer hover:bg-white/20 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/50 min-w-[70px] lg:min-w-[80px]"
                        ...
                        <div className="flex items-center justify-center">
                          <span className="text-gray-500 text-xs lg:text-sm px-2 lg:px-3 py-1.5 lg:py-2 bg-white/5 rounded-lg border border-white/10">
                            Done
                          </span>
                        </div>
                      )}
                  ...
                  >
                    ...
                    <td className="p-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {canUpdateStatus(order.orderStatus) ? (
                          <select
                            className="bg-white/10 border border-white/20 rounded-lg px-2 py-1.5 text-white text-xs cursor-pointer"
                            value={order?.orderStatus}
                            onChange={(e) =>
                              handleStatusChange(order, e.target.value)
                            }
          ...
          orders.map((order: any, index: number) => (
            ...
            >
              ...
              <div className="flex gap-2 sm:gap-3">
                ...
                {canUpdateStatus(order.orderStatus) ? (
                  <select
                    className="bg-white/10 border border-white/20 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-white text-xs sm:text-sm cursor-pointer hover:bg-white/20 transition-colors min-w-[80px]"
                    value={order?.orderStatus}
                  ...
                  <div className="flex items-center justify-center px-3 sm:px-4 bg-white/5 rounded-xl border border-white/10">
                    <span className="text-gray-500 text-xs sm:text-sm">Done</span>
                  </div>
                )}
              </div>
            </motion.div>
        ...
        {otpModel && (
          ...
          >
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 border border-white/20 rounded-2xl p-4 sm:p-6 w-full max-w-sm sm:max-w-md shadow-2xl"
            >
              <div className="flex justify-between items-start mb-4 sm:mb-6">
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className="p-1.5 sm:p-2 bg-green-500/20 rounded-lg">
                    <FaCheckCircle className="w-4 sm:w-5 h-4 sm:h-5 text-green-400" />
                  </div>
                  <h2 className="text-base sm:text-xl font-bold">Enter Delivery OTP</h2>
                </div>
                <button
                  onClick={() => {
                    setOtpModel(null);
              ...
              <p className="text-xs sm:text-sm text-gray-400 mb-3 sm:mb-4 flex items-center gap-1.5 sm:gap-2">
                <FaHashtag className="w-3 sm:w-4 h-3 sm:h-4" />
                Order ID:{" "}
                <span className="text-white font-semibold">
                  #{String(otpModel._id).slice(-6).toUpperCase()}
                </span>
              </p>

              <p className="text-xs sm:text-sm text-gray-300 mb-4 sm:mb-6 bg-blue-500/10 border border-blue-500/30 rounded-lg p-2.5 sm:p-3">
                OTP has been sent to the buyer's email. Please ask the customer
                for the OTP.
              </p>

              <input
                type="text"
                className="w-full bg-white/10 border border-white/20 px-3 sm:px-4 py-3 sm:py-4 rounded-xl text-white text-center text-lg sm:text-xl tracking-widest font-mono mb-4 sm:mb-6 focus:outline-none focus:ring-2 focus:ring-green-500/50 focus:border-green-500/50"
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 py-3 sm:py-4 rounded-xl flex items-center justify-center gap-2 text-white font-semibold transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-green-500/30 transform hover:-translate-y-1 text-sm sm:text-base"
              >
                {isLoading ? (
                  ...
                  <>
                    ...
                    <span className="hidden xs:inline">Verify & Complete Delivery</span>
